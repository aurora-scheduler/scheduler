...
class GeneratedCode(object):
    ...
    def dump(self, out_file):
        ...
        def remove_by_prefix(prefix):
            ...
            remaining_imports[:] = [
                i for i in remaining_imports if not i.startswith(prefix)]
            return group
...
def parse_structs(thrift_defs):
    ...
    def parse_field(field):
        ...
        if type_params:
            params = [make_type(p)
                      for p in type_params.replace(' ', '').split(',')]
...
def generate_union_field(code, struct, field):
    field_enum_value = '%s._Fields.%s' % (
        struct.name, to_upper_snake_case(field.name))
...
def generate_struct_field(code, field, builder_calls):
    ...
    if field.ttype.immutable:
        ...
        if isinstance(field.ttype, ParameterizedType):
            ...
            field_type = 'Immutable%s<%s>' % (
                field.ttype.name, field.ttype.param_names())
    ...
    if isinstance(field.ttype, StructType):
        ...
        assert field.ttype.name in [
            'List', 'Map', 'Set'], 'Unrecognized type %s' % field.ttype.name
        code.add_import('com.google.common.collect.Immutable%s' %
                        field.ttype.name)
        ...
        if all([p.immutable for p in params]):
            ...
            builder_assignment = '%s.toMutableBuilders%s(%s)' % (
                params[0].codegen_name(), field.ttype.name, field.name)
        else:
    ...
    nullable = field.ttype.name == 'String' or not isinstance(
        field.ttype, (PrimitiveType, ParameterizedType))
    if nullable:
        code.add_accessor(FIELD_TEMPLATE % {'type': 'boolean',
                                            'fn_name': field.isset_method(),
                                            'field': '%s != null' % field.name})
        builder_calls.append('.set%s(%s == null ? null : %s)' % (
            field.capitalized_name(), field.name, builder_assignment))
    else:
        builder_calls.append('.set%s(%s)' %
                             (field.capitalized_name(), builder_assignment))
...
def generate_java(struct):
    ...
    if struct.kind == 'union':
        ...
        for field in struct.fields:
            ...
            assert field.ttype.immutable or isinstance(
                field.ttype, StructType), 'Unrecognized type %s' % field.ttype.name

            if field.ttype.immutable:
                assign_case_body = 'value = wrapped.%s();\nbreak;' % field.accessor_method()
                copy_case_body = 'return new %s(setField, %s());' % (
                    struct.name, field.accessor_method())
                ...
                copy_case_body = 'return new %s(setField, %s().newBuilder());' % (
                    struct.name, field.accessor_method())
        ...
        code.add_accessor(FIELD_TEMPLATE % {
                          'type': set_field_type, 'fn_name': 'getSetField', 'field': 'setField'})
        code.add_field(FIELD_DECLARATION %
                       {'field': 'setField', 'type': set_field_type})
        ...
        code.add_accessor(UNION_COPY_CONSTRUCTOR_2 %
                          {'wrapped': struct.name, 'body': copy_2_switch})
        ...
        code.copy_constructor = 'return new %s()%s;' % (
            struct.name, '\n        ' + '\n        '.join(builder_calls))
        code.to_string = '\n        ' + \
            '\n        '.join(['.add("%s", %s)' % (f, f) for f in field_names])
        code.equals = '\n        && '.join(
            ['Objects.equals(%s, other.%s)' % (f, f) for f in field_names])
        code.hash_code = '\n          ' + \
...
if __name__ == '__main__':
    ...
    if len(args) != 3:
        print('usage: %s thrift_file code_output_dir resource_output_dir' %
              sys.argv[0])
        sys.exit(1)
    ...
    with open(thrift_file) as f:
        ...
        if not services:
            log('Skipping generation for %s since there are no services.' %
                thrift_file)
            sys.exit(0)
        structs = parse_structs(file_contents)

        package_dir = os.path.join(
            code_output_dir, PACKAGE_NAME.replace('.', os.path.sep))
        ...
        for struct in structs:
            ...
            if struct.kind == 'enum':
                continue
            gen_file = os.path.join(package_dir, '%s.java' %
                                    struct.codegen_name())
        ...
        resource_dir = os.path.join(
            resource_output_dir, PACKAGE_NAME.replace('.', os.path.sep), 'help')
